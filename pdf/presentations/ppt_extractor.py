...
class PPTExtractor(Extractor):
    ...
    def extract(
        self, content: Content, params: PPTExtractorConfig
        ...
        for slide_idx, slide in enumerate(prs.slides):
            ...
            for shape in slide.shapes:
                if shape.has_text_frame:
                    ...
                    contents.append(
                        Content.from_text(
                            "\n".join(text_output),
                            features=[Feature.metadata({"page": slide_idx})],
                        )
                    )
            ...
            for shape in slide.shapes:
                if shape.has_table:
                    ...
                    contents.append(
                        Content.from_json(
                            table_data, features=[Feature.metadata({"page": slide_idx})]
                        )
                    )
    ...
    def sample_input(self) -> Content:
        req = requests.get(
            "https://pub-157277cc11d64fb1a11f71cc52c688eb.r2.dev/figures.pptx"
        )
