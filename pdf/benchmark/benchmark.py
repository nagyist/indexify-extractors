...
def create_and_run_extraction_graph(graph_name, extractor_name, pdf_filepath):
    ...
    extracted_content = client.get_extracted_content(
        content_id=content_id, graph_name=graph_name, policy_name="pdf_to_text"
    )
...
for pdf_file in os.listdir(pdf_dir):
    if pdf_file.endswith(".pdf"):
        ...
        marker_output = create_and_run_extraction_graph(
            "markerbench", "tensorlake/marker", pdf_path
        )
        marker_time = time.time() - start_time
        marker_score = score_text(marker_output, reference_text)
        print(
            f"Marker score for {pdf_file}: {marker_score} (Time taken: {marker_time:.2f} seconds)"
        )

        # Run PDF extractor
        start_time = time.time()
        pdf_extractor_output = create_and_run_extraction_graph(
            "pdfbench", "tensorlake/pdf-extractor", pdf_path
        )
        pdf_extractor_time = time.time() - start_time
        pdf_extractor_score = score_text(pdf_extractor_output, reference_text)
        print(
            f"PDF extractor score for {pdf_file}: {pdf_extractor_score} (Time taken: {pdf_extractor_time:.2f} seconds)"
        )

        # Run Unstructured IO extractor
        start_time = time.time()
        unstructured_output = create_and_run_extraction_graph(
            "unstructuredbench", "tensorlake/unstructuredio", pdf_path
        )
        unstructured_time = time.time() - start_time
        unstructured_score = score_text(unstructured_output, reference_text)
        print(
            f"Unstructured IO score for {pdf_file}: {unstructured_score} (Time taken: {unstructured_time:.2f} seconds)"
        )

        # Run EasyOCR extractor
        start_time = time.time()
        easyocr_output = create_and_run_extraction_graph(
            "easyocrbench", "tensorlake/easyocr", pdf_path
        )
        easyocr_time = time.time() - start_time
        easyocr_score = score_text(easyocr_output, reference_text)
        print(
            f"Easy OCR score for {pdf_file}: {easyocr_score} (Time taken: {easyocr_time:.2f} seconds)"
        )

        # Run OCRMyPDF extractor
        start_time = time.time()
        ocrmypdf_output = create_and_run_extraction_graph(
            "ocrmypdfbench", "tensorlake/ocrmypdf", pdf_path
        )
        ocrmypdf_time = time.time() - start_time
        ocrmypdf_score = score_text(ocrmypdf_output, reference_text)
        print(
            f"OCR My PDF score for {pdf_file}: {ocrmypdf_score} (Time taken: {ocrmypdf_time:.2f} seconds)"
        )
